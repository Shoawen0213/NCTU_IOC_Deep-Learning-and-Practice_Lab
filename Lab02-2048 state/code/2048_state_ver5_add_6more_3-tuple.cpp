...
int main(int argc, const char* argv[]) {
    ...
    tdl.add_feature(new pattern({ 0, 1, 2, 4, 6, 8}));
    ...
    tdl.add_feature(new pattern({ 0, 1, 5}));
    tdl.add_feature(new pattern({ 0, 4, 5}));
    tdl.add_feature(new pattern({ 11, 14, 15}));
    tdl.add_feature(new pattern({ 8, 12, 13}));
    tdl.add_feature(new pattern({ 10, 13, 14}));
	...
	//save score 
	std::ofstream output;
	output.open("score_td0_ver5.txt");
    int ver = 0;
	...
	for (size_t n = 1; n <= total; n++) {
        ...
        // store the model ezch 10000 episode
        if(n%10000==0){
            ver++;
			std::stringstream ss;
			std::string strVer;
